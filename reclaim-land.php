...
session_start();
  ...
  if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $add_reclaim_query = "insert into reclaimations (farmer_name,land_area,crop_type,money,address,investor_name,investor_phone,image) values ('$farmer_name','$land_area','$crop_type','$money','$address','',0,'$image')";
    $add_reclaim = mysqli_query($con, $add_reclaim_query);

    if($add_reclaim) {
      $error_msg =  "Successfully added your reclaimation.";
    } else {
        $error_msg =  "error adding your reclaimation, try again later!";
    }
  } 
...
<body>
  ...
  <header id="header" class="fixed-top">
    ...
    <div class="container mt-3" style="max-width: 700px;"><br>
        <h3>Request an Investor to reclaim your land</h3>
        <form method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
            <div class="row mb-3">
                ...
                <div class="col">
                    <input type="number" class="form-control" placeholder="Money Needed (in EGP)" name="money" required>
                    
                </div>
            </div>

            <input type="text" class="form-control mb-4" placeholder="Crop Type" name="crop_type" required>
